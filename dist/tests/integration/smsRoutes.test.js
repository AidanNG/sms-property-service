...
describe('SMS Routes', () => {
    ...
    describe('POST /sms/incoming', () => {
        ...
        it('should handle valid address lookup successfully', async () => {
            ...
            geocodeAddress.mockResolvedValue(mockGeo);
            getPropertyData.mockResolvedValue(mockProperty);
            sendPropertyEmail.mockResolvedValue(undefined);
            const response = await request(app)
                .post('/sms/incoming')
        ...
        it('should handle property not found', async () => {
            const mockGeo = { lat: 123, lon: 456, display_name: '123 Test St' };
            geocodeAddress.mockResolvedValue(mockGeo);
            getPropertyData.mockResolvedValue(null);
            sendPropertyEmail.mockResolvedValue(undefined);
            const response = await request(app)
                .post('/sms/incoming')
                .send({ Body: '123 Test St', From: '+1234567890' });
        ...
        it('should handle service errors', async () => {
            geocodeAddress.mockRejectedValue(new Error('Service error'));
            const response = await request(app)
                .post('/sms/incoming')
                .send({ Body: '123 Test St', From: '+1234567890' });
